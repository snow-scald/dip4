...
public:
	...
	inline static const WFloat ZERO = WFloat(0); // 定义0
	inline static const WFloat ONE = WFloat(1);  // 定义1
	inline static const WFloat TEN = WFloat(10); // 定义10（用于除法化整）
...
inline void WFloat::trim()
{
	...
	auto iter = integer.rbegin(); // 对整数部分
...
inline WFloat::WFloat(const string &num) // 用字符串初始化，格式形如"-123.456"、"1.0"
{
	...
	for(auto iter = num.crbegin(); iter < num.crend(); iter++) // 逆向迭代
	{
		char ch = (*iter);
